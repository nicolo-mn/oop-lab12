...
namespace OperatorsOverloading
{
    ...
    public abstract class List<TValue>
    {
        ...
        {
            TValue[] arr = new TValue[list.Length];
            int i = 0;
        ...
        public static bool operator ==(List<TValue> list1, List<TValue> list2)
        {
            ...
            var arrList1 = (TValue[])list1;
            var arrList2 = (TValue[])list2;
            for (int i = 0; i < arrList1.Length; i++)
            {
                if (!arrList1[i].Equals(arrList2[i]))
                {
                    return false;
                }
            }
        ...
        {
            List<TValue> list = List.From(list1.ToFlat());
            return List.From(list.ToFlat().Where(x => !list2.ToFlat().Contains(x)));
        }
